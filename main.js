  ...
  function isEn(){ return typeof window.getLang === 'function' && window.getLang() === 'en'; }
  function closeMenu(){
    if(!navToggle || !mobileMenu) return;
    navToggle.classList.remove('is-open');
    navToggle.setAttribute('aria-expanded', 'false');
    navToggle.setAttribute('aria-label', isEn() ? 'Open menu' : 'Abrir menú');
  ...
  function openMenu(){
    ...
    navToggle.setAttribute('aria-label', isEn() ? 'Close menu' : 'Cerrar menú');
  ...
  (function initContactForms(){
    ...
    document.querySelectorAll('.cform[action]:not(#form-rrhh)').forEach(form => {
      form.addEventListener('submit', async (e) => {
        ...
        const en = isEn();
        btn.disabled = true;
        btn.textContent = en ? 'Sending…' : 'Enviando…';
        setStatus(form, '', '');
        ...
        try {
          ...
          setStatus(form, 'ok', en ? 'Thanks! We received your message and will get back to you shortly.' : '¡Gracias! Recibimos tu mensaje y te vamos a responder a la brevedad.');
          form.reset();
        } catch (err) {
          console.error('Contacto:', err);
          setStatus(form, 'err', en ? 'There was a problem sending it. Please try again or email us directly.' : 'Hubo un problema al enviar. Probá de nuevo o escribinos directamente por mail.');
        } finally {
          btn.disabled = false;
    ...
    if (location.search.includes('enviado=rrhh')) {
      ...
      if (rrhhForm) setStatus(rrhhForm, 'ok', isEn() ? "Thanks! We received your application and CV — we'll be in touch if an opportunity comes up." : '¡Gracias! Recibimos tu postulación y tu CV — te contactamos si surge una oportunidad.');
